        ...
        extends AbstractTranscriptionService
{
        ...
        implements StreamingRecognitionSession
    {
        ...
        public void onMessage(String msg)
        {
            ...
            long startMicro = (long)(obj.get("start").getAsFloat() * 1_000_000);
            boolean partial = !obj.get("is_final").getAsBoolean();
            JsonObject alternative = obj.getAsJsonObject("channel")
                    .get("alternatives").getAsJsonArray()
                    .get(0).getAsJsonObject();
            String result = alternative.get("transcript").getAsString();
            ...
            if (!result.isEmpty() && (!partial || !result.equals(lastResult)))
            {
                ...
                for (TranscriptionListener l : listeners)
                {
                    l.notify(new TranscriptionResult(
                            ...
                            partial,
                            transcriptionTag,
                            alternative.get("confidence").getAsFloat(),
